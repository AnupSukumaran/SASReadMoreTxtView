...
public class SASReadMoreTxtView: UITextView {
    ...
    @IBOutlet public weak var readMoreDelegate: TextViewTapActionProtocol?
    ...
    public override func layoutSubviews() {
       ...
       if _needsUpdateTrim {
        ...
        txtBold = UIFont.boldSystemFont(ofSize: 16)
        txtColor = .blue
        
        let tapGest = UITapGestureRecognizer(target: self, action: #selector(tapGestAction))
        self.addGestureRecognizer(tapGest)
       needsTrim() ? showLessText() : ()
    }
    
    @objc func tapGestAction() {
